...
const org_bubble_width = 1500;
const org_bubble_title_height = 45;
...
const calling_bubble_vertical_margin = 20;
const min_margin_between_calling_bubbles = 20;
...
pub fn generate_diagram_file_contents(allocator: std.mem.Allocator, orgs: std.StringArrayHashMap(data.Organization)) ![]const u8 {
    ...
    const calling_bubble_min_row_width = calling_bubble_dimensions.width * calling_bubbles_per_row;
    const calling_bubble_left_margin_in_org_bubble = (org_bubble_width - calling_bubble_min_row_width) / (calling_bubbles_per_row + 1);
    ...
    // TODO: Validate bubbles will fit with min_margin_between_calling_bubbles or throw

    var cursor_x: i32 = diagram_start_x;
    var cursor_y: i32 = diagram_start_y;
    ...
    for (org_ordering) |ordering| {
        ...
        if (start_new_column) {
            cursor_x += org_bubble_width + org_bubble_margins.right + org_bubble_margins.left;
            cursor_y = diagram_start_y;
        }

        cursor_y += org_bubble_margins.top;
        const org_bubble_x = cursor_x;
        const org_bubble_y = cursor_y;
        ...
        cursor_y += org_bubble_title_height + calling_bubble_vertical_margin;
        ...
        for (org.callings.items) |calling| {
            ...
            if (i != 0) {
                if (i % calling_bubbles_per_row == 0) {
                    cursor_x = org_bubble_x;
                    cursor_y += calling_bubble_dimensions.height + calling_bubble_vertical_margin;
                } else {
                    cursor_x += calling_bubble_dimensions.width;
                }
            }

            cursor_x += calling_bubble_left_margin_in_org_bubble;
        ...
        if (org.callings.items.len > 0) {
            cursor_y += calling_bubble_vertical_margin;
        }

        cursor_x = org_bubble_x;

        const org_bubble_height = cursor_y - org_bubble_y;
        ...
        cursor_y += org_bubble_margins.bottom;
    }
